...
class ProcessTest(unittest.TestCase) :
    ...
    def test_low_SMP_are_not_kanji(self) :
        code = random.randint(0x10000, 0x101FF)
        char = chr(code)
        message = f"Character '{char}' should not be kanji"
        assert not process.isKanji(char), message
    ...
    def test_decompose_heisei_era_name(self) :
        era_name_char = chr(0x337B)
        expected = chr(0x5E73) + chr(0x6210)
        actual = process.decompose(era_name_char)
        message = f"Decomposing '{era_name_char}'"
        self.assertEqual(expected, actual, message)

    def test_decompose_syouwa_era_name(self) :
        era_name_char = chr(0x337C)
        expected = chr(0x662D) + chr(0x548C)
        actual = process.decompose(era_name_char)
        message = f"Decomposing '{era_name_char}'"
        self.assertEqual(expected, actual, message)
    ...
    def test_decompose_meizi_era_name(self) :
        era_name_char = chr(0x337E)
        expected = chr(0x660E) + chr(0x6CBB)
        actual = process.decompose(era_name_char)
        message = f"Decomposing '{era_name_char}'"
        self.assertEqual(expected, actual, message)

    def test_decompose_corporation_symbol(self) :
        symbol = chr(0x337F)
        expected = chr(0x682A) + chr(0x5F0F) + chr(0x4F1A) + chr(0x793E)
        actual = process.decompose(symbol)
        message = f"Decomposing '{symbol}'"
        self.assertEqual(expected, actual, message)
